...
# Page 2: Conditional Questions
    ...
    if st.button("Submit ✅"):
        new_row = pd.DataFrame([[job_role, years_experience, org_size, st.session_state.uses_9box, usage_frequency if st.session_state.uses_9box == "Yes" else "N/A", primary_purpose if st.session_state.uses_9box == "Yes" else "N/A", ease_of_use if st.session_state.uses_9box == "Yes" else "N/A", stakeholders if st.session_state.uses_9box == "Yes" else "N/A", talent_retention if st.session_state.uses_9box == "Yes" else "N/A", succession_usefulness if st.session_state.uses_9box == "Yes" else "N/A", fairness if st.session_state.uses_9box == "Yes" else "N/A", challenges if st.session_state.uses_9box == "Yes" else "N/A", recommend_9box if st.session_state.uses_9box == "Yes" else "N/A", assessment_methods if st.session_state.uses_9box != "Yes" else "N/A", reason_not_using if st.session_state.uses_9box != "Yes" else "N/A", structured_effectiveness if st.session_state.uses_9box != "Yes" else "N/A", adopt_future if st.session_state.uses_9box != "Yes" else "N/A", additional_comments, email]], columns=["Job Role", "Years of Experience", "Organization Size", "Uses 9-Box Grid", "Usage Frequency", "Primary Purpose", "Ease of Use", "Stakeholders", "Talent Retention", "Succession Usefulness", "Fairness", "Challenges", "Recommend 9-Box", "Assessment Methods", "Reason Not Using", "Structured Effectiveness", "Adopt Future", "Additional Comments", "Email"])
